...
define(['N/error', 'N/record', 'N/ui/serverWidget', 'N/search', 'N/file'],
    function (error, record, ui, search, file) {
        ...
        var MAX_ITEM_RECEIPT_ALLOWED_TO_UPDATE = 10;
            ...
            if (context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT ||
                ...
                if (context.type == context.UserEventType.EDIT) {
                    ...
                    if (LANDED_COST_RELCALC_NOT_ALLOWED_STATUSES.indexOf(status) > -1) {
                        ...
                        sublist.getField({id: "custpage_costamount"}).updateDisplayType({
                            displayType: ui.FieldDisplayType.DISABLED
                        });
                    }
                }
                ...
                if (context.type == context.UserEventType.VIEW || context.type == context.UserEventType.EDIT) {
                    // Adjusted Landed Cost UI
                    status = context.newRecord.getValue({fieldId: "shipmentstatus"});
                    if (status == "received") {
                        createAdjustLandedcostUI(context);
                    } else {
                        form.getField({id: "custrecord_fmt_adjlandedcost"}).updateDisplayType({
                            displayType: ui.FieldDisplayType.DISABLED
                        });
                    }
                }
        ...
        function createAdjustLandedcostUI(context) {
            var form = context.form;
            var lineNo = 0;
            var sublist = form.addSublist({
                id: "custpage_adjsublist",
                type: ui.SublistType.LIST,
                label: "Update Landed Costs",
                tab: "landedcost_tab"
            });

            sublist.addField({
                id: "custpage_costcategoryadj",
                type: ui.FieldType.SELECT,
                label: "Cost Category",
                source: "costcategory"
            });
            ...
            var catList = getCostCategoryList();
            ...
            // Adjusted Landed Cost UI
            var landedCostDataId = getLandedCostDataIdAdjusted(context.newRecord.id);
            if (!!landedCostDataId) {
                var lcRec = record.load({type: "customrecord_landcostmaster", id: landedCostDataId});
                var sublistCount = sublist.lineCount;
                var currentLineCostCat, cLineIndex, costCatAmount;
        ...
        function afterSubmit(context) {
            if (context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT) {
                ...
                var adjustLandedCost = ibsUpdRec.getValue({fieldId: "custrecord_fmt_adjlandedcost"});
                ...
                if (!landedCostCalcualted) {
                    ...
                    if (!!itemsData && Object.keys(itemsData).length > 0) {
                        ...
                        //6- Finally Saving the Record
                        ibsUpdRec.save();
                    }
                }

                //------------------------------ Landed cost adjustment start-----------------
                if (adjustLandedCost) {
                    //6- Save Landed Cost Data to Custom Record (Adjusted)
                    log.debug("Save Landed Cost Data to Custom Record Adjusted");
                    saveLandedCostDataAdjusted(ibsRec);
                    itemsData = prepareLandedCostLinesData(ibsRec, ibsUpdRec);
                    log.debug("itemsData", JSON.stringify(itemsData));
                    var itemWiseUnitLandedCost = getItemWiseUnitLandedCost(ibsRec, ibsUpdRec, itemsData);
                    log.debug("itemWiseUnitLandedCost", JSON.stringify(itemWiseUnitLandedCost));
                    //TODO : Unmark the adjust landed cost checkbox
                    var isReceiptIds = getISItemReceiptIds(context.newRecord.id), item, itemQty, lcSubRec, itemCostData;
                    var lcLineCount, lclFoundIndex;
                    var itemReceiptRec, commitLine;
                    log.debug("isReceiptIds", JSON.stringify(isReceiptIds));
                    if (!!isReceiptIds && isReceiptIds.length > 0 && isReceiptIds.length <= MAX_ITEM_RECEIPT_ALLOWED_TO_UPDATE) {
                        for (var r = 0; r < isReceiptIds.length; r++) {
                            itemReceiptRec = record.load({
                                type: 'itemreceipt',
                                id: isReceiptIds[r],
                                isDynamic: true
                            });
                            ...
                            for (var i = 0; i < lineCount; i++) {
                                ...
                                item = itemReceiptRec.getCurrentSublistValue({
                                    sublistId: "item",
                                    fieldId: "item",
                                    line: i
                                });
                                ...
                                try {
                                    ...
                                    itemCostData = itemWiseUnitLandedCost[item];
                                    ...
                                    log.debug("Unable to set landed cost for item receipt " +
                                        isReceiptIds[r] + " for line : " + i);
                                }
                            }
                            if (commitLine) itemReceiptRec.save();
                        }
                        ibsUpdRec = record.load({type: context.newRecord.type, id: context.newRecord.id});
                        ibsUpdRec.setValue({fieldId: "custrecord_fmt_adjlandedcost", value: false});
                        ibsUpdRec.save();
                    }

                }
                //------------------------------ Landed cost adjustment end-------------------
        ...
        // Add/Update Landed Cost Lines by Cost Category
        function getItemWiseUnitLandedCost(ibsRec, ibsUpdRec, itemsData) {
            var totalLCLines = ibsRec.getLineCount({sublistId: "custpage_adjsublist"});
            var totalCCLandedCost, costCatagory, itemCostingData = {}, itemQuantity;
            var cbmPct;
            if (!!itemsData) {
                for (var c = 0; c < totalLCLines; c++) {
                    totalCCLandedCost = ibsRec.getSublistValue({
                        sublistId: "custpage_adjsublist",
                        fieldId: "custpage_costamountadj",
                        line: c
                    });
                    costCatagory = ibsRec.getSublistValue({
                        sublistId: "custpage_adjsublist",
                        fieldId: "custpage_costcategoryadj",
                        line: c
                    });

                    for (var i in itemsData) {
                        cbmPct = itemsData[i].cbmPct;
                        if (!itemCostingData [i]) itemCostingData [i] = [];
                        totalCCLandedCost = parseFloat(totalCCLandedCost);
                        totalCCLandedCost = isNaN(totalCCLandedCost) ? 0 : totalCCLandedCost;
                        itemQuantity = itemsData[i].quantity;
                        itemCostingData [i].push(
                            {
                                costCategroy: costCatagory,
                                unitRate: (totalCCLandedCost * cbmPct) / itemQuantity
                            }
                        );
                    }
                }
        ...
        //Function to save Landed Cost Data from Temporary UI to Custom Reccord
        function saveLandedCostDataAdjusted(ibsRec) {
            var totalLCLines = ibsRec.getLineCount({sublistId: "custpage_adjsublist"});
            var lcRec, existingLandedCostRecId, lcRecLinesCount
            ...
            if (!!totalLCLines) {
                existingLandedCostRecId = getLandedCostDataIdAdjusted(ibsRec.id);
                ...
                if (!!existingLandedCostRecId) {
                    ...
                    lcRec = record.create({type: "customrecord_landcostmaster", isDynamic: true});
                    lcRec.setValue({fieldId: "custrecord_fmt_ibs", value: ibsRec.id});
                    lcRec.setValue({fieldId: "custrecord_fmt_adjusted", value: true});
                }
                ...
                log.debug("Saving the customrecord_landcostmaster record (Adjusted)");
                if (!!lcRec) lcRec.save();
            }
        }
        ...
        function getLandedCostDataId(isId) {
            ...
            var customrecord_landcostmasterSearchObj = search.create({
                type: "customrecord_landcostmaster",
                filters: [
                    ["custrecord_fmt_ibs", "is", isId], "and", ["custrecord_fmt_adjusted", "is", "F"]
                ]
            });
        ...
        //Get Stored LandedCost data by InboundShipment Id (Adjusted)
        function getLandedCostDataIdAdjusted(isId) {
            var lcdId;
            var customrecord_landcostmasterSearchObj = search.create({
                type: "customrecord_landcostmaster",
                filters: [
                    ["custrecord_fmt_ibs", "is", isId], "and", ["custrecord_fmt_adjusted", "is", "T"]
                ]
            });
        ...
        function getISItemReceiptIds(isId) {
            var itemReceipts = [];
            var inboundshipmentSearchObj = search.create({
                type: "inboundshipment",
                filters:
                    [
                        ["itemreceipt.linesequencenumber", "equalto", "1"],
                        "AND",
                        ["internalid", "anyof", isId]
                    ],
                columns:
                    [
                        search.createColumn({
            ...
            inboundshipmentSearchObj.run().each(function (result) {
                itemReceipts.push(result.getValue({name: "internalid", join: "itemReceipt"}));
                return true;
            });
            return itemReceipts;
        }
