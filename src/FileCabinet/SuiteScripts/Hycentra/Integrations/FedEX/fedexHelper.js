...
define(['N/runtime', 'N/record', 'N/format', 'N/https', 'N/error', 'N/log'],
	function(runtime, record, format, https, error, log) {
		...
		function buildRecipientInfo(fulfillmentRecord) {
            ...
            return {
                "contact": {
                    "personName": "James Lui",
                    "phoneNumber": "9498188528"
                },
                "address": {
                    "streetLines": ["530 Technology Drive", "Suite 100"],
                    "city": "Irvine",
			...
			// Individual fields are empty, parse the formatted shipaddress field
			var formattedAddress = fulfillmentRecord.getValue({fieldId: 'shipaddress'}) || '';
			if (!formattedAddress) {
				throw error.create({
					name: 'MISSING_SHIPPING_ADDRESS',
					message: 'No shipping address found in individual fields or shipaddress field'
				});
			}
