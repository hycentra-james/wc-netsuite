...
define(['N/runtime', 'N/record', 'N/format', 'N/https', 'N/error', 'N/log', 'N/file', 'N/search', 'N/url', '../../../Concentrus/PackShipTemplate/Con_Lib_Print_Node.js', '../kitCartonResolver.js'],
    function (runtime, record, format, https, error, log, file, search, url, printNodeLib, kitCartonResolver) {
        ...
        function buildPackageLineItems(fulfillmentRecord, mappingRecord, shipMethodMapping) {
            ...
            try {
                ...
                // Step 3b: Resolve the sold kit(s) carton layout (WC-648). Combination/kit items
                // are packed by WMS as component inventory items, but the true box dimensions live
                // on the sold kit's carton SKUs (custitem_wc_carton_sku_N). buildKitBoxDefs throws
                // on a mis-configured kit so we never print a label with wrong dimensions.
                var kitLayout = kitCartonResolver.buildKitBoxDefs(fulfillmentId);
